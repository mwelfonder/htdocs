...
// Define directory paths specific to this file
define('FILE_SPECIFIC_PRIMARY_DIR', '/var/www/vhosts/scan4-gmbh.de/crm.scan4-gmbh.de/uploads/');
define('FILE_SPECIFIC_FALLBACK_DIR', '/var/www/html/uploads/');
...
function processHBGRequest()
{
    ...
    if (isset($_POST['image'])) {
        ...
        function isValidBase64($string)
        {
            ...
            // Check if decode is successful and the decoded data is not false
            return $decoded !== false && base64_encode($decoded) === $string;
        }

        if (isValidBase64($base64_string)) {
            // If it's valid base64, use it directly
            $imageFilePath = processImageData($homeid, $base64_string);
        } else {
            // Check if the string contains the expected delimiter ';'
            if (strpos($base64_string, ';') !== false) {
                list($type, $data) = explode(';', $base64_string, 2);

                // Check if the string contains the expected delimiter ','
                if (strpos($data, ',') !== false) {
                    list(, $data) = explode(',', $data, 2);

                    // Now validate the extracted base64 data
                    if (isValidBase64($data)) {
                        $imageFilePath = processImageData($homeid, $data);
                    } else {
                        error_log("Error: Invalid base64 data after extraction.");
                    }
                } else {
                    // Handle the error - delimiter ',' not found
                    error_log("Error: ',' delimiter not found in image data.");
                }
            } else {
                // Handle the error - delimiter ';' not found
                error_log("Error: ';' delimiter not found in base64 string.");
            }
        }
    }
    ...
    if ($id) {
        ...
        $stmt->bind_param("ssssi", $reason, $isExcelInt, $comment, $filePath, $id);
        $stmt->execute();
        if ($stmt->error) {
            $response = ["success" => false, "message" => "MySQL Error: " . $stmt->error];
        }
        ...
        if ($stmt->affected_rows === 0) {
            ...
            // $response = ["success" => true, "message" => "Database updated"];
        }

        $stmt->close();
    } else {
        $response = ["success" => false, "message" => "Id is not provided, cant update db"];
    }
...
function process_surveyImages($imageData, $imgId, $homeid)
{
    ...
    $subDirPath = 'scan4App/surveyData/images/';

    // Check if primary directory exists, else use the fallback with the same subdirectory structure
    $baseDir = is_dir(FILE_SPECIFIC_PRIMARY_DIR . $subDirPath) ? FILE_SPECIFIC_PRIMARY_DIR . $subDirPath : FILE_SPECIFIC_FALLBACK_DIR . $subDirPath;

    // Append the current year to the base directory
    $year = date("Y"); // Get the current year
    $targetDir = $baseDir . $year . "/";
    ...
    while (file_exists($targetFile)) {
        $targetFile = $targetDir . $baseFileName . '_i' . $counter
            . ".jpg";
        $counter++;
...
function processImageData($homeid, $imageData)
{
    ...
    // Define the subdirectory path
    $subDirPath = 'scan4App/abbruch/';

    // Check if primary directory exists, else use the fallback with the same subdirectory structure
    $baseDirectory = is_dir(FILE_SPECIFIC_PRIMARY_DIR . $subDirPath) ? FILE_SPECIFIC_PRIMARY_DIR . $subDirPath : FILE_SPECIFIC_FALLBACK_DIR . $subDirPath;

    // Construct the year-specific directory
    $year = date("Y"); // Get the current year
...
function process_SurveyDataRequest()
{
    ...
    if ($surveyData && $homeid && $username) {
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(["success" => false, "message" => "Failed to store survey data. MySQL Error: " . $stmt->error]);
        }
...
function processFileUpload($homeid, $city)
{
    ...
    // Fetch the uploaded file's temporary location
    ...
    // Define the subdirectory path
    $subDirPath = 'hbgprotokolle/begehungen/screenshots/';

    // Check if primary directory exists, else use the fallback with the same subdirectory structure
    $baseDir = is_dir(FILE_SPECIFIC_PRIMARY_DIR . $subDirPath) ? FILE_SPECIFIC_PRIMARY_DIR . $subDirPath : FILE_SPECIFIC_FALLBACK_DIR . $subDirPath;

    // Construct the directory paths based on the base directory
    $yearDir = $baseDir . date('Y');
    $cityDir = $yearDir . '/' . $city;
    $destinationPath = $cityDir . '/' . $newFileName;

    // Create year and city directories if they don't exist
    if (!is_dir($yearDir)) {
    ...
    if (move_uploaded_file($uploadedFile, $destinationPath)) {
        return $destinationPath; // Return the path on successful upload
    }
