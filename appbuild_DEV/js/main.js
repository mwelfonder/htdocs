...
const serverEndpoint = 'https://crm-node-1-app.scan4-gmbh.com/upload.php';
...
class Appointments {
    ...
    fetchAppointmentsFromServer(targetDate = this.todayDate(), retryCount = 0) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: response => {
                console.log('fetchAppointmentsFromServer', response)
    ...
    uploadToServer(dataToSend, date, index) {
        ...
        return new Promise((resolve, reject) => {
            ...
            xhr.onerror = function () {
                console.error('Error uploading data:', xhr.statusText);
                reject({ success: false });
                element.find('.progress').hide(); // Hide progress bar on error
            };
...
function print_populatefields(surveyData, appointmentData, imageData, modifiedImageData) {
    ...
    const idToSelectorMap = {
        ...
        'hausnummer_preview3': '#bp_img_haus > div:eq(3)',
